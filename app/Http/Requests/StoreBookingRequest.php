    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            $quotation = Quotation::query()
                ->withCount('items')
                ->withCount('booking')
                ->with('inquiry:id,handled_by,assigned_to,created_by')
                ->find($quotationId);
            ...
            if (! in_array(Quotation::normalizeStatus((string) $quotation->status), [Quotation::STATUS_APPROVED], true)) {
                $validator->errors()->add('quotation_id', ui_phrase('Booking can only be created from an approved quotation.'));
            }
