...
- Itinerary detail PDF preview/download must follow the active language selected by the user session, not a separate language selector inside the PDF button. The PDF view/controller must use `ui_phrase()` for all visible system text and enum/display labels, including item type, transport type, duration units, empty states, alt text, driver/AC values, and default tagline. User-entered itinerary content is preserved as stored unless a dedicated content-translation field exists. Simplified/Traditional Chinese output must use bundled CJK fonts registered through DomPDF `FontMetrics`, with `storage/fonts` available for DomPDF cache files, so translated labels render as real glyphs instead of square placeholders.
...
- Itinerary index filters must follow the module index baseline: one compact filter card, AJAX filter/pagination via `data-service-filter-*`, `title` as the only min-3 text filter, `destination_id`, numeric `duration`, and `per_page`. Duration must not inherit text minimum-character rules; use input-level `data-filter-min-text="0"` where a form-level min text is present.
- Activities index filters must follow the module index baseline with `data-service-filter-*`, KPI cards, one compact filter card, and controller-prepared row payloads (`activityRows`) for desktop/mobile reuse. The active filter set is `q`, `vendor_id`, `activity_type_id`, `status`, and `per_page`; `q` follows the min-3 text rule.
- Vendors / Providers index filters must follow the module index baseline with KPI cards, one compact filter card, AJAX filter/pagination via `data-service-filter-*`, and active filters limited to `q`, `service_type`, `status`, and `per_page`. Do not add a separate `Vendor Type` filter because saved vendor types remain display metadata while operational filtering is handled by `Service Type`.
- Item Validation Queue is an independent controllable module with key `item_validation_queue`. It must show every unvalidated service item or provider created directly from itinerary create/edit quick-add flows, including Activity, F&B, Tourist Attraction, and auto-created Vendor/Provider drafts. Queue routes keep `itineraries.manual_item_queue.*` action permissions for compatibility, but module availability must be controlled by `module:item_validation_queue`. When any queued Activity, F&B, Tourist Attraction, or Vendor/Provider is updated from its edit page, the matching pending queue log must be marked validated/resolved so it disappears from the queue.
- Quotation detail `Workflow Visibility` must respect downstream module gates. `Booking Status` and operation status appear only when the `bookings` module is enabled, while `Invoice Status`, `Payment Status`, invoice/payment current-stage messaging, next-action messaging, and invoice/payment risk indicators appear only when the `invoices` module is enabled. Payment screens currently run under the invoice module gate, so payment visibility follows `invoices`.
...
- Quotation PDF must follow the active language selected by the user session. The PDF view/controller must use `ui_phrase()` for all visible system text and enum/display labels, including title, source, duration units, service type, totals, version, service date, pax label, and menu label. User-entered quotation item descriptions and customer/itinerary names remain as stored unless translated content fields are added.
- Simplified/Traditional Chinese quotation PDF output must use bundled CJK fonts registered through DomPDF `FontMetrics`, with `storage/fonts` available for DomPDF cache files, so translated labels render as real glyphs instead of square placeholders.
