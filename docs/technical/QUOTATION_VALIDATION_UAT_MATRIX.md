...
| 4 | Reservation/Manager/Director | Klik description item di halaman validasi | Modal detail terbuka dan memuat data via endpoint JSON (lazy load), bukan preload semua item. |
...
| 7 | Reservation/Manager/Director | Klik `Validate Selected Items` | Item terpilih berubah menjadi tervalidasi jika data rate valid. |
| 8 | Reservation/Manager/Director | Klik `Final Validate Quotation` saat belum semua item tervalidasi | Ditolak dengan pesan bahwa validasi belum selesai. |
| 9 | Reservation/Manager/Director | Klik `Final Validate Quotation` saat semua item wajib tervalidasi | `validation_status` quotation menjadi `valid`, `validated_at/by` terisi. |
...
2. Detail modal item menggunakan lazy API JSON per klik description.
