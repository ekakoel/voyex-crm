...
@php\n$kpiCards = [];
    if ($canBookings) {
        $kpiCards[] = ['label' => ui_phrase('My Revenue This Month'), 'value' => $kpis['monthly_revenue'] ?? 0, 'icon' => 'wallet', 'color' => 'emerald', 'format' => 'money'];
    }
    if ($canBookings && $canInquiries) {
        $kpiCards[] = ['label' => ui_phrase('My Conversion Rate'), 'value' => $kpis['conversion_rate'] ?? 0, 'icon' => 'chart-line', 'color' => 'indigo', 'suffix' => '%'];
    }
    if ($canInquiries) {
        $kpiCards[] = ['label' => ui_phrase('My Active Inquiries'), 'value' => $kpis['active_inquiries'] ?? 0, 'icon' => 'circle-question', 'color' => 'sky'];
        $kpiCards[] = ['label' => ui_phrase('Overdue Follow-ups'), 'value' => $kpis['overdue_followups'] ?? 0, 'icon' => 'calendar-times', 'color' => 'rose'];
    }
@endphp

<div class="sa-wrap rounded-3xl border border-slate-200/80 bg-slate-100/70 p-3 dark:border-slate-700 dark:bg-slate-900/60">
    @section('page_title', ui_phrase('Marketing Dashboard'))
    @section('page_subtitle', ui_phrase('Monitor your personal sales performance and follow-ups.'))
    @section('page_actions')
        <div class="flex items-center gap-2">
            @if($canCustomers)
                <a href="{{ route('customers.create') }}"  class="btn-secondary-sm">
                    <i class="fa-solid fa-user-plus mr-2"></i>{{ ui_phrase('New Customer') }}
                </a>
            @endif
            @if($canInquiries)
                <a href="{{ route('inquiries.create') }}"  class="btn-primary-sm">
                    <i class="fa-solid fa-plus-circle mr-2"></i>{{ ui_phrase('New Inquiry') }}
                </a>
            @endif
        </div>
    @endsection
    ...
    <div class="space-y-3">
        ...
        @if($canInquiries || $canQuotations || $canBookings)
            <div class="grid grid-cols-1 gap-3 lg:grid-cols-2">
                <div class="sa-card p-5">
                    <h2 class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ ui_phrase('My Sales Funnel') }}</h2>
                ...
                @if($canInquiries)
                    <div class="sa-card p-5">
                        <h2 class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ ui_phrase('My Inquiry Status') }}</h2>
                         <div class="mt-3 grid grid-cols-2 gap-x-3 gap-y-2">
                            @forelse($inquiryByStatus as $status => $total)
                                <div class="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs dark:border-slate-700 dark:bg-slate-900">
                                    <x-status-badge :status="$status" :label="ui_phrase((string) $status)" size="xs" />
        ...
        @if($canInquiries)
            <div class="grid grid-cols-1 gap-3 lg:grid-cols-2">
                <div class="sa-card p-4">
                    <h3 class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ ui_phrase('Action Center: My Follow-ups') }}</h3>
                    <div class="mt-3 space-y-2">
                        ...
                        @empty
                             <div class="rounded-lg mb-6 border border-dashed border-slate-200 p-4 text-center dark:border-slate-700">
                                 <p class="text-xs font-medium text-slate-600 dark:text-slate-300">{{ ui_phrase('You have no upcoming follow-ups.') }}</p>
                            </div>
                        @endforelse
                    </div>
                </div>
                 ...
                 <div class="sa-card p-5">
                    <div class="flex items-center justify-between">
                        <h2 class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ ui_phrase('My Recent Inquiries') }}</h2>
                            ...
                            <a href="{{ route('inquiries.show', $inquiry) }}"  class="block rounded-xl border border-slate-200 bg-white px-3 py-2 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:hover:bg-slate-800/50">
                                <div class="flex items-center justify-between text-xs">
                                 ...
                                 <div class="mt-1 flex items-center justify-between text-xs">
                                    <span class="text-slate-500 dark:text-slate-400">{{ ui_phrase('Customer:') }} {{ $inquiry->customer->name ?? ui_phrase('N/A') }}</span>
                                    <span class="text-slate-500 dark:text-slate-400">{{ \App\Support\DateTimeDisplay::date($inquiry->created_at) }}</span>
                                </div>
                            </a>
                        @empty
                            <p class="text-center text-xs text-slate-500 dark:text-slate-400">{{ ui_phrase('You have no recent inquiries.') }}</p>
                        @endforelse
                    </div>
                </div>
            </div>
