...
@section('content')
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.quotation-templates.create') }}" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700">Add Template</a>
        </div>
            ...
            @forelse ($templates as $template)
                <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <div class="mt-3 flex flex-wrap gap-2">
                        <a href="{{ route('admin.quotation-templates.edit', $template) }}" class="rounded-lg border border-gray-300 px-3 py-1 text-xs font-medium text-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-700">Edit</a>
                        <form action="{{ route('admin.quotation-templates.destroy', $template) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                    ...
                    @forelse ($templates as $template)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                            ...
                            <td class="px-4 py-3 text-right text-sm">
                                <a href="{{ route('admin.quotation-templates.edit', $template) }}" class="mr-3 font-medium text-indigo-600 hover:text-indigo-700 dark:text-indigo-400">Edit</a>
                                <form action="{{ route('admin.quotation-templates.destroy', $template) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
