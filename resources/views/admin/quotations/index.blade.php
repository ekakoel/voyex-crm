    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('admin.quotations.export', request()->query()) }}" class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-700">
                    Export CSV
                </a>
                <a href="{{ route('admin.quotations.create') }}" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700">
                    Add Quotation
                </a>
            </div>
        </div>
        ...
        <form method="GET" class="grid grid-cols-1 gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 md:grid-cols-6">
            ...
            <div class="flex flex-col gap-2 md:col-span-6 sm:flex-row">
                ...
                <a href="{{ route('admin.quotations.index') }}" class="w-full sm:w-auto rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-700 text-center">Reset</a>
            </div>
        </form>
            ...
            @forelse ($quotations as $quotation)
                <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    <div class="flex items-start justify-between gap-3">
                        ...
                        <span class="text-xs font-medium rounded-full bg-gray-100 px-2 py-0.5 text-gray-700 dark:bg-gray-900/40 dark:text-gray-300">
                            {{ $quotation->status }}
                        </span>
                    </div>
                    ...
                    <div class="mt-3 flex flex-wrap gap-2">
                        <a href="{{ route('admin.quotations.edit', $quotation) }}" class="rounded-lg border border-gray-300 px-3 py-1 text-xs font-medium text-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-700">Edit</a>
                        <a href="{{ route('admin.quotations.pdf', $quotation) }}" class="rounded-lg border border-emerald-300 px-3 py-1 text-xs font-medium text-emerald-700 hover:bg-emerald-50 dark:border-emerald-700 dark:text-emerald-300 dark:hover:bg-emerald-900/20">PDF</a>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    @forelse ($quotations as $quotation)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                            ...
                            <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-200">{{ $quotation->status }}</td>
                            ...
                            <td class="px-4 py-3 text-right text-sm">
                                <a href="{{ route('admin.quotations.edit', $quotation) }}" class="mr-3 font-medium text-indigo-600 hover:text-indigo-700 dark:text-indigo-400">Edit</a>
                                <a href="{{ route('admin.quotations.pdf', $quotation) }}" class="mr-3 font-medium text-emerald-600 hover:text-emerald-700 dark:text-emerald-400">PDF</a>
                                <form action="{{ route('admin.quotations.destroy', $quotation) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
