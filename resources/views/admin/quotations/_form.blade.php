...
<div class="space-y-5">
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-200">Promo Code</label>
            <input
                name="promo_code"
                type="text"
                value="{{ old('promo_code', $quotation->promo_code ?? '') }}"
    ...
    <div class="rounded-xl border border-gray-200 p-4 dark:border-gray-700">
        ...
        <div class="mt-3 space-y-3">
            @for ($i = 0; $i < max($minRows, count($items)); $i++)
                @php
                    $row = $items[$i] ?? ['service_id' => '', 'description' => '', 'qty' => 1, 'unit_price' => 0, 'discount' => 0];
                @endphp
                <div class="grid grid-cols-1 gap-3 rounded-lg border border-dashed border-gray-200 p-3 dark:border-gray-700 sm:grid-cols-6">
                    <div class="sm:col-span-2">
                        <label class="block text-xs text-gray-500">Service</label>
                        <select name="items[{{ $i }}][service_id]" class="mt-1 w-full rounded-lg border border-gray-300 px-2 py-2 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100">
                            <option value="">-</option>
                            @foreach ($services as $service)
                                <option value="{{ $service->id }}" @selected((string) ($row['service_id'] ?? '') === (string) $service->id)>
                                    {{ $service->name }}
                                </option>
                            @endforeach
                        </select>
    ...
    <div class="flex items-center gap-2">
        ...
        <a href="{{ route('admin.quotations.index') }}" class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-700">
            Cancel
        </a>
    </div>
</div>
