...
<div class="module-card p-6 space-y-4">
    <div class="flex items-center justify-between gap-2">
        <p class="text-xs font-semibold uppercase tracking-wide text-gray-600 dark:text-gray-300">{{ ui_phrase('Comments') }}</p>
    ...
    @if ($hasNewComment)
        <div class="rounded-lg mb-6 border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700 dark:border-amber-700 dark:bg-amber-900/20 dark:text-amber-300">
            {{ ui_phrase('There is a new comment from :name.', ['name' => $latestComment->user?->name ?? ui_phrase('Unknown')]) }}
        </div>
    @endif
    ...
    <div class="space-y-3">
            ...
            <div class="rounded-lg mb-6 border border-gray-200 p-3 text-xs text-gray-700 dark:border-gray-700 dark:text-gray-200 {{ $index === 0 ? 'bg-indigo-50/60 dark:bg-indigo-900/20' : '' }}">
                <div class="flex items-center justify-between gap-2">
                    <div class="font-semibold text-gray-800 dark:text-gray-100">
                        {{ $comment->user?->name ?? ui_phrase('Unknown') }}
                    </div>
                    <div class="flex items-center gap-2 text-[11px] text-gray-500 dark:text-gray-400">
                        @if ($index === 0)
                            <span class="rounded-full bg-indigo-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-indigo-700 dark:bg-indigo-800/60 dark:text-indigo-200">{{ ui_phrase('Newest') }}</span>
                        @endif
                ...
                @if ($editingId === (int) $comment->id && $user && (int) ($comment->user_id ?? 0) === (int) $user->id)
                    <form method="POST" action="{{ route('quotations.comments.update', [$quotation, $comment]) }}" class="mt-2 space-y-2">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <a href="{{ url()->current() }}" class="rounded-lg border border-gray-300 px-3 py-1.5 text-[11px] font-medium text-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-700">{{ ui_phrase('Cancel') }}</a>
                        </div>
                    </form>
                @else
                    @php
                ...
                @if ($user && (int) ($comment->user_id ?? 0) === (int) $user->id && ! $quotation->isFinal())
                    <div class="mt-2 flex items-center gap-2">
                        <a href="{{ url()->current() . '?edit_comment_id=' . $comment->id }}" class="text-[11px] font-medium text-indigo-600 hover:text-indigo-700 dark:text-indigo-300">{{ ui_phrase('Edit') }}</a>
                        <form method="POST" action="{{ route('quotations.comments.destroy', [$quotation, $comment]) }}" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" onclick="return confirm('{{ ui_phrase('Delete this comment?') }}')" class="btn-danger-sm">
                                {{ ui_phrase('Delete') }}
                            </button>
                        </form>
                    </div>
                    ...
                    <div class="mt-3 space-y-2 border-l-2 border-emerald-200 pl-3 dark:border-emerald-800">
                        @foreach ($replies as $reply)
                            <div class="rounded-lg border border-emerald-200/80 bg-emerald-50/50 p-2 text-xs dark:border-emerald-800 dark:bg-emerald-900/10">
                                <div class="flex items-center justify-between gap-2">
                                    <div class="font-semibold text-gray-800 dark:text-gray-100">{{ $reply->user?->name ?? ui_phrase('Unknown') }}</div>
                ...
                @if ($isCreator && ! $quotation->isFinal() && $replyingToId === (int) $comment->id)
                    <form method="POST" action="{{ route('quotations.comments.store', $quotation) }}" class="mt-3 space-y-2 rounded-lg border border-emerald-200 bg-emerald-50/40 p-3 dark:border-emerald-800 dark:bg-emerald-900/10">
                        ...
                        <textarea
                            ...
                            placeholder="{{ ui_phrase('Write a reply to this comment...') }}"
                        ...
                        @enderror
                        <div class="flex items-center gap-2">
                            <button type="submit" class="btn-primary-sm">{{ ui_phrase('Send Reply') }}</button>
                            <a href="{{ url()->current() }}" class="rounded-lg border border-gray-300 px-3 py-1.5 text-[11px] font-medium text-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-700">{{ ui_phrase('Cancel') }}</a>
                        </div>
                    </form>
                @endif
        ...
        @empty
            <p class="text-xs text-gray-500 dark:text-gray-400">{{ ui_phrase('No comments yet.') }}</p>
        @endforelse
    </div>
    ...
    @if ($canComment)
        <details class="rounded-lg border border-gray-200 p-3 dark:border-gray-700">
            <summary class="cursor-pointer text-xs font-semibold text-gray-700 dark:text-gray-200">
                <i class="fa-solid fa-comment-dots mr-2"></i> {{ ui_phrase('Add Comment') }}
            </summary>
            <form method="POST" action="{{ route('quotations.comments.store', $quotation) }}" class="mt-2 space-y-2">
                ...
                <textarea
                    ...
                    rows="3"
                    class="w-full app-input"
                    placeholder="{{ ui_phrase('Write a comment for this quotation...') }}"
                >{{ old('comment_body') }}</textarea>
                @error('comment_body')
                    <p class="text-xs text-rose-600">{{ $message }}</p>
                @enderror
                <button type="submit" class="btn-primary-sm">
                    {{ ui_phrase('Add Comment') }}
                </button>
            </form>
        </details>
    @endif
