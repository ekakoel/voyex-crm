...
@section('page_title', ui_phrase('editor_dashboard_page_title'))
@section('page_subtitle', ui_phrase('editor_dashboard_page_subtitle'))
@section('page_actions')
    @if ($canManualQueue)
        <a href="{{ route('itineraries.manual-item-validation-queue') }}" class="btn-primary">{{ ui_phrase('editor_dashboard_actions_open_item_validation_queue') }}</a>
    @endif
@endsection
    ...
    <div class="space-y-6 module-page module-page--editor-dashboard">
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
            <div class="app-card p-5">
                <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ ui_phrase("$t.cards.need_validation.title") }}</p>
                <p class="mt-2 text-2xl font-semibold text-gray-900 dark:text-gray-100">
                    {{ number_format((int) ($pendingManualItemsCount ?? 0)) }}
                </p>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ ui_phrase("$t.cards.need_validation.caption") }}</p>
            </div>

            <div class="app-card p-5">
                <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ ui_phrase("$t.cards.validated_today.title") }}</p>
                <p class="mt-2 text-2xl font-semibold text-gray-900 dark:text-gray-100">
                    {{ number_format((int) ($myValidatedTodayCount ?? 0)) }}
                </p>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ ui_phrase("$t.cards.validated_today.caption") }}</p>
            </div>

            <div class="app-card p-5 md:col-span-2 xl:col-span-2">
                <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ ui_phrase("$t.cards.priority.title") }}</p>
                @if (($pendingManualItemsCount ?? 0) > 0)
                    <p class="mt-2 text-sm font-medium text-amber-700 dark:text-amber-300">
                        {{ ui_phrase("$t.cards.priority.waiting_message", ['count' => number_format((int) $pendingManualItemsCount)]) }}
                    </p>
                @else
                    <p class="mt-2 text-sm font-medium text-emerald-700 dark:text-emerald-300">
                        {{ ui_phrase("$t.cards.priority.empty_message") }}
                    </p>
                @endif
            </div>
        </div>
        ...
        <div class="grid min-w-0 grid-cols-1 gap-6 xl:grid-cols-12">
            <section class="min-w-0 space-y-4 xl:col-span-8">
                <div class="app-card p-5">
                    <div class="flex items-center justify-between gap-3">
                        <div>
                            <h2 class="text-base font-semibold text-gray-800 dark:text-gray-100">{{ ui_phrase("$t.recent_pending.title") }}</h2>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ ui_phrase("$t.recent_pending.subtitle") }}</p>
                        </div>
                        @if ($canManualQueue)
                            <a href="{{ route('itineraries.manual-item-validation-queue') }}" class="btn-outline-sm">{{ ui_phrase("$t.recent_pending.view_queue") }}</a>
                        @endif
                    </div>
                    ...
                    @if (($recentPendingManualItems ?? collect())->isEmpty())
                        <div class="mt-4 rounded-lg border border-gray-200 bg-gray-50 p-4 text-sm text-gray-600 dark:border-gray-700 dark:bg-gray-900/30 dark:text-gray-300">
                            {{ ui_phrase("$t.recent_pending.empty") }}
                        </div>
                    @else
                            ...
                            @foreach ($recentPendingManualItems as $log)
                                ...
                                <div class="rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-800/40">
                                    ...
                                    <div class="mt-1 flex flex-wrap items-center justify-between gap-2">
                                        <p class="text-xs text-gray-600 dark:text-gray-300">{{ ui_phrase("$t.recent_pending.created_by") }} {{ $creatorName }}</p>
                                        @if ($editUrl !== '')
                                            <a href="{{ $editUrl }}" target="_blank" rel="noopener" class="btn-ghost-sm">{{ ui_phrase("$t.recent_pending.open_item") }}</a>
                                        @endif
                                    </div>
                                </div>
                            @endforeach
                ...
                <div class="app-card p-5">
                    <h2 class="text-base font-semibold text-gray-800 dark:text-gray-100">{{ ui_phrase("$t.recent_validated.title") }}</h2>
                    @if (($recentlyValidatedByMe ?? collect())->isEmpty())
                        <p class="mt-3 text-sm text-gray-500 dark:text-gray-400">{{ ui_phrase("$t.recent_validated.empty") }}</p>
                    @else
            ...
            <aside class="min-w-0 space-y-4 xl:col-span-4">
                <div class="app-card p-5">
                    <h3 class="text-sm font-semibold text-gray-800 dark:text-gray-100">{{ ui_phrase("$t.content_scope.title") }}</h3>
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ ui_phrase("$t.content_scope.subtitle") }}</p>
                    ...
                    <div class="mt-4 space-y-2">
                        ...
                        @empty
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ ui_phrase("$t.content_scope.empty") }}</p>
                        @endforelse
                    </div>
                </div>
            </aside>
