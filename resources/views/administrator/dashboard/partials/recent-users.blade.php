@if($canUsers)
    <div>
        <h3 class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ ui_phrase('administrator_dashboard_sections_recently_updated_users') }}</h3>
        <div class="mt-3 space-y-2">
            @forelse($recentUsers as $user)
                @php
                    ...
                    if (in_array($normalizedName, $systemRoleTokens, true)) {
                        $displayName = ui_term($normalizedName);
                    }
                @endphp
            ...
            @empty
                <p class="text-xs text-slate-500 dark:text-slate-400" data-progressive-item>{{ ui_phrase('administrator_dashboard_recent_users_empty') }}</p>
            @endforelse
        </div>
    </div>
@endif
