    ...
    <div>
        ...
        <div class="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            @foreach($systemKpis as $card)
                <a href="{{ isset($card['route']) && Route::has($card['route']) ? route($card['route']) : '#' }}" class="sa-kpi sa-kpi-sm" data-progressive-item>
                    <div class="flex items-center justify-between">
...
@else
    <p class="text-sm text-slate-500 dark:text-slate-400" data-progressive-item>No access to system management KPI.</p>
@endif
