    ...
    <div>
        ...
        <div class="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
            @foreach($masterDataKpis as $card)
                <a href="{{ isset($card['route']) && Route::has($card['route']) ? route($card['route']) : '#' }}" class="block rounded-xl border border-slate-200 bg-slate-50 p-3 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-800/50 dark:hover:bg-slate-800" data-progressive-item>
...
@else
    <p class="text-sm text-slate-500 dark:text-slate-400" data-progressive-item>No access to master data KPI.</p>
@endif
