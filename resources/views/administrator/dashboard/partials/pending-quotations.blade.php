@if($canQuotations)
    <div>
        <h3 class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ ui_phrase('Pending Quotations') }}</h3>
            ...
            @forelse($pendingQuotations as $quotation)
                <a href="{{ route('quotations.show', $quotation) }}" class="block rounded-lg bg-slate-50 px-3 py-2 text-xs hover:bg-slate-100 dark:bg-slate-800/50 dark:hover:bg-slate-800" data-progressive-item>
                    ...
                    <p class="text-slate-500 dark:text-slate-400">
                        {{ ui_phrase('Customer:') }} {{ $quotation->inquiry?->customer?->name ?? ui_phrase('N/A') }}
                    </p>
                </a>
            @empty
                <p class="text-xs text-slate-500 dark:text-slate-400" data-progressive-item>{{ ui_phrase('No pending quotations.') }}</p>
            @endforelse
        </div>
    </div>
@endif
