...
@php\n$kpiCards = [];
    if ($canInvoices) {
        $kpiCards = [
            ['label' => ui_phrase('Monthly Total'), 'value' => $monthlyTotal ?? 0, 'icon' => 'wallet', 'color' => 'emerald', 'format' => 'money'],
            ['label' => ui_phrase('Pending Invoices'), 'value' => $pendingInvoices ?? 0, 'icon' => 'file-invoice', 'color' => 'amber'],
            ['label' => ui_phrase('Overdue'), 'value' => $overdueInvoices ?? 0, 'icon' => 'triangle-exclamation', 'color' => 'rose'],
        ];
    }
@endphp
...
<div class="sa-wrap rounded-3xl border border-slate-200/80 bg-slate-100/70 p-3 dark:border-slate-700 dark:bg-slate-900/60" data-progressive-dashboard>
    @section('page_title', ui_phrase('Finance Dashboard'))
    @section('page_subtitle', ui_phrase('Monitor invoices and payment flow.'))
    ...
    <div class="grid grid-cols-1 gap-3 xl:grid-cols-12" data-progressive-group>
                    ...
                    @foreach($kpiCards as $card)
                        <div class="sa-kpi app-kpi-card" data-progressive-item>
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-[10px] text-slate-400">{{ ui_phrase('live') }}</span>
                            </div>
                            <p>{{ $card['label'] }}</p>
                            <b>
        ...
        <aside  class="xl:col-span-4 space-y-3" data-progressive-group>
            <div class="sa-card p-4" data-progressive-item>
                <h3 class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ ui_phrase('Recent Invoices') }}</h3>
                <div class="mt-3 space-y-2 text-xs">
                    @forelse($recentInvoices as $invoice)
                        <div class="rounded-lg bg-slate-50 px-3 py-2 dark:bg-slate-900" data-progressive-item>
                            ...
                            <p class="text-slate-500 dark:text-slate-400">{{ ui_phrase((string) $invoice->status) }} � <x-money :amount="$invoice->total_amount" currency="IDR" /></p>
                        </div>
                    @empty
                        <p class="text-xs text-slate-500 dark:text-slate-400">{{ ui_phrase('No recent invoices.') }}</p>
                    @endforelse
                </div>
            </div>
        </aside>
