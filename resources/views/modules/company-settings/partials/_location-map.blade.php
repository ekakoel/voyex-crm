<div class="space-y-2">
    <h3 class="text-sm font-semibold text-gray-800 dark:text-gray-100">{{ ui_phrase('settings location on map') }}</h3>
    ...
    @push('scripts')
        ...
        <script>
            function initCompanyLocationMap(root = document) {
                const mapHintMarkerMoved = @json(ui_phrase('settings map hint marker moved'));
                const mapHintInvalid = @json(ui_phrase('settings map hint invalid'));
                const mapHintSuccess = @json(ui_phrase('settings map hint success'));
