...
@section('content')
    <div class="space-y-6 module-page module-page--quotations">
        @section('page_actions')<a href="{{ route('quotations.show', $quotation) }}" class="rounded-lg border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-700">
                    View Detail
                </a>@endsection
                        ...
                        @if (auth()->user()->hasAnyRole(['Director']))
                            <form method="POST" action="{{ route('quotations.approve', $quotation) }}" class="mt-3 space-y-3">
                                ...
                                <div class="flex items-center gap-2">
                                    @if (($quotation->status ?? '') !== 'approved')
                                        <button type="submit" formaction="{{ route('quotations.approve', $quotation) }}" class="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-medium text-white hover:bg-emerald-700">Approve</button>
                                        <button type="submit" formaction="{{ route('quotations.reject', $quotation) }}" class="rounded-lg bg-rose-600 px-3 py-2 text-xs font-medium text-white hover:bg-rose-700">Reject</button>
                                    @else
                                        <button type="submit" formaction="{{ route('quotations.set-pending', $quotation) }}" class="rounded-lg border border-amber-300 px-3 py-2 text-xs font-medium text-amber-700 hover:bg-amber-50 dark:border-amber-700 dark:text-amber-300 dark:hover:bg-amber-900/20">
                                            Set Pending
                                        </button>
                                    @endif
                                </div>
