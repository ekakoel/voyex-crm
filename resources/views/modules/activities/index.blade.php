...
@endsection
@section('content')
    <div class="space-y-6 module-page module-page--activities" data-service-filter-page data-page-spinner="off">
        <div class="module-grid-main" data-service-filter-results>
                <div class="grid grid-cols-2 gap-3 xl:grid-cols-3">
                    @foreach (collect($statsCards ?? [])->take(3) as $stat)
                        <x-ui.metric-card :title="(string) ($stat['label'] ?? '-')" :value="(string) ($stat['value'] ?? 0)" />
                    @endforeach
                </div>
                <div class="app-card p-5">
                    <form method="GET" action="{{ route('activities.index') }}" class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-6" data-service-filter-form data-filter-min-text="3" data-disable-submit-lock="1" data-page-spinner="off">
                        <input name="q" value="{{ request('q') }}" placeholder="{{ ui_phrase('Search (min 3 characters)') }}" class="app-input sm:col-span-2 lg:col-span-2" data-service-filter-input data-filter-min-text="3">
                        ...
                        <select name="activity_type_id" class="app-input" data-service-filter-input>
                        ...
                        <div class="flex items-center gap-2 sm:col-span-2 lg:col-span-6 filter-actions h-[42px]">
                            <a href="{{ route('activities.index') }}" class="btn-secondary h-[42px] rounded-[var(--app-radius-sm)] px-4" data-service-filter-reset>{{ ui_phrase('Reset') }}</a>
                        </div>
                    </form>
                </div>
                <div>
                    @include('modules.activities.partials._index-results', [
                        'activities' => $activities,
                        'activityRows' => $activityRows,
                        'canManageActivationActions' => $canManageActivationActions,
                    ])
                </div>
        </div>
    </div>
