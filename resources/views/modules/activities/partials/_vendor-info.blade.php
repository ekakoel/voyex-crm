    ...
    <dl class="mt-3 space-y-2">
        @if (filled($vendor?->name))
            <div class="grid grid-cols-[110px_1fr] gap-2">
                <dt class="text-xs text-slate-500 dark:text-slate-400">Vendor Name</dt>
                <dd class="text-sm text-slate-700 dark:text-slate-200 break-words">{{ $vendor->name }}</dd>
            </div>
        @endif
        @if (filled($vendor?->contact_name))
            <div class="grid grid-cols-[110px_1fr] gap-2">
                <dt class="text-xs text-slate-500 dark:text-slate-400">Contact Name</dt>
