...
@section('page_title', ui_phrase('modules_food_beverages_page_title'))
@section('page_subtitle', ui_phrase('modules_food_beverages_create_page_subtitle'))
        ...
        <div class="module-grid-8-4">
                ...
                <div class="module-form-wrap">
                    @if (isset($copiedFrom) && $copiedFrom)
                        <div class="mb-4 rounded-lg border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-700 dark:border-sky-700/70 dark:bg-sky-900/20 dark:text-sky-200">
                            {!! ui_phrase('modules_food_beverages_copied_from', ['name' => '<span class="font-semibold">'.e($copiedFrom->name).'</span>']) !!}
                        </div>
                    @endif
                    <form method="POST" action="{{ route('food-beverages.store') }}" enctype="multipart/form-data">
                        @csrf
                        @include('modules.food-beverages._form', ['buttonLabel' => ui_phrase('modules_food_beverages_save'), 'prefill' => $prefill ?? []])
                    </form>
                </div>
            </div>
            <aside class="module-grid-side">
                <div class="module-card p-5 text-sm text-slate-600 dark:text-slate-300">
                    ...
                    <p class="mt-2">{{ ui_phrase('modules_food_beverages_info_text') }}</p>
                </div>
            </aside>
        </div>
    </div>
