...
<div class="space-y-6 hotel-form">
    <div class="app-card p-5 space-y-4">
        <div class="flex items-center justify-between">
            <h3 class="text-sm font-semibold text-gray-800 dark:text-gray-100">{{ ui_phrase('Hotel Prices') }}</h3>
        ...
        <div id="price-rows" class="space-y-3">
            @foreach ($priceRows as $index => $row)
                <div class="grid grid-cols-1 gap-3 rounded-lg border border-slate-200 p-3 dark:border-slate-700 md:grid-cols-12" data-row>
                    ...
                    <div class="md:col-span-4">                        <label class="block text-xs text-gray-500">{{ ui_phrase('Start Date') }}</label>
                        <input type="date" name="hotel_prices[{{ $index }}][start_date]" value="{{ $row['start_date'] ?? '' }}" class="mt-1 app-input">
                    </div>
                    <div class="md:col-span-4">                        <label class="block text-xs text-gray-500">{{ ui_phrase('End Date') }}</label>
                        <input type="date" name="hotel_prices[{{ $index }}][end_date]" value="{{ $row['end_date'] ?? '' }}" class="mt-1 app-input">
                    </div>
                    <div class="md:col-span-3">                        <label class="block text-xs text-gray-500">{{ ui_phrase('Contract Rate (IDR)') }}</label>
                    ...
                    <div class="md:col-span-3">                        <label class="block text-xs text-gray-500">{{ ui_phrase('Markup Type') }}</label>
                        <select name="hotel_prices[{{ $index }}][markup_type]" data-hotel-rate="markup_type" class="mt-1 app-input">
                            ...
                            <option value="percent" @selected(($row['markup_type'] ?? 'fixed') === 'percent')>{{ ui_phrase('Percent') }}</option>
                        </select>
                    </div>
                    <div class="md:col-span-3">                        <label class="block text-xs text-gray-500">{{ ui_phrase('Markup') }}</label>
                    ...
                    <div class="md:col-span-3">                        <label class="block text-xs text-gray-500">{{ ui_phrase('Publish Rate (Auto)') }}</label>
                    ...
                    <div class="md:col-span-12 flex justify-end">
                        <button type="button" class="mt-1 btn-ghost-sm h-[38px] w-full md:w-auto" data-remove-row>{{ ui_phrase('Remove') }}</button>
                    </div>
                </div>
            @endforeach
        </div>
