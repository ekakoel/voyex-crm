...
<div class="space-y-6 hotel-form">
    <div class="app-card p-5 space-y-4">
        <div class="flex items-center justify-between">
            <h3 class="text-sm font-semibold text-gray-800 dark:text-gray-100">{{ ui_phrase('Hotel Rooms') }}</h3>
            <button type="button" class="btn-ghost-sm" data-add-row="room">{{ ui_phrase('Add Room') }}</button>
        </div>
            ...
            @foreach ($roomRows as $index => $row)
                ...
                <div class="app-card p-4" data-room-card>
                    <div class="flex flex-wrap items-center justify-between gap-3">
                        ...
                        <button type="button" class="btn-ghost-sm" data-remove-room>{{ ui_phrase('Remove') }}</button>
                    </div>
                        ...
                        <div class="md:col-span-8">
                            <div class="mt-4 grid grid-cols-2 gap-3 md:grid-cols-12">
                                <div class="md:col-span-6">
                                    <label class="block text-xs text-gray-500">{{ ui_phrase('Room Name') }}</label>
                                    <input name="rooms[{{ $index }}][rooms]" value="{{ $row['rooms'] ?? '' }}" class="mt-1 app-input" required data-room-name>
                                </div>
                                <div class="md:col-span-6">
                                    <label class="block text-xs text-gray-500">{{ ui_phrase('Room View') }}</label>
                    ...
                    <div class="mt-4 grid grid-cols-1 gap-3 md:grid-cols-12">
                        <div class="md:col-span-6">
                            <label class="block text-xs text-gray-500">{{ ui_phrase('Amenities') }}</label>
                            <textarea name="rooms[{{ $index }}][amenities]" rows="2" class="mt-1 app-input">{{ $row['amenities'] ?? '' }}</textarea>
                        </div>
                        <div class="md:col-span-6">
                            <label class="block text-xs text-gray-500">{{ ui_phrase('Include') }}</label>
                            <textarea name="rooms[{{ $index }}][include]" rows="2" class="mt-1 app-input">{{ $row['include'] ?? '' }}</textarea>
                        </div>
                        <div class="md:col-span-6">
                            <label class="block text-xs text-gray-500">{{ ui_phrase('Additional Info') }}</label>
            ...
            <div class="app-card p-4" data-room-card>
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm font-semibold text-gray-800 dark:text-gray-100" data-room-title>{{ ui_phrase('Room NUMBER') }}</span>
                    </div>
                    <button type="button" class="btn-ghost-sm" data-remove-room>{{ ui_phrase('Remove') }}</button>
                </div>
                <div class="mt-4 grid grid-cols-1 gap-3 md:grid-cols-12">
