...
<div class="space-y-6 hotel-form">
    <div class="app-card p-5 space-y-4">
        <div class="flex items-center justify-between">
            <h3 class="text-sm font-semibold text-gray-800 dark:text-gray-100">{{ __('Hotel Rooms') }}</h3>
            <button type="button" class="btn-ghost-sm" data-add-row="room">{{ __('Add Room') }}</button>
        </div>
            ...
            @foreach ($roomRows as $index => $row)
                ...
                <div class="app-card p-4" data-room-card>
                    <div class="flex flex-wrap items-center justify-between gap-3">
                        ...
                        <button type="button" class="btn-ghost-sm" data-remove-room>{{ __('Remove') }}</button>
                    </div>
                        ...
                        <div class="md:col-span-4">
                            @php
                                ...
                                $coverFull = $coverStoredPath !== '' && ! $coverIsExternal
                                    ...
                                    : $coverStoredPath;
                            @endphp
                            <label class="block text-xs text-gray-500">{{ __('Cover (Upload Image)') }}</label>
                        ...
                        <div class="md:col-span-8">
                            <div class="mt-4 grid grid-cols-2 gap-3 md:grid-cols-12">
                                <div class="md:col-span-6">
                                    <label class="block text-xs text-gray-500">{{ __('Room Name') }}</label>
                                    <input name="rooms[{{ $index }}][rooms]" value="{{ $row['rooms'] ?? '' }}" class="mt-1 app-input" required data-room-name>
                                </div>
                                <div class="md:col-span-6">
                                    <label class="block text-xs text-gray-500">{{ __('Room View') }}</label>
                    ...
                    <div class="mt-4 grid grid-cols-1 gap-3 md:grid-cols-12">
                        <div class="md:col-span-6">
                            <label class="block text-xs text-gray-500">{{ __('Amenities') }}</label>
                            <textarea name="rooms[{{ $index }}][amenities]" rows="2" class="mt-1 app-input">{{ $row['amenities'] ?? '' }}</textarea>
                        </div>
                        <div class="md:col-span-6">
                            <label class="block text-xs text-gray-500">{{ __('Include') }}</label>
                            <textarea name="rooms[{{ $index }}][include]" rows="2" class="mt-1 app-input">{{ $row['include'] ?? '' }}</textarea>
                        </div>
                        <div class="md:col-span-6">
                            <label class="block text-xs text-gray-500">{{ __('Additional Info') }}</label>
        ...
        <template id="room-row-template">
            <div class="app-card p-4" data-room-card>
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div class="flex items-center gap-2">
                        <span class="inline-flex items-center rounded-full bg-slate-100 px-2 py-0.5 text-xs font-semibold text-slate-700">{{ __('__NUMBER__') }}</span>
                        <span class="text-sm font-semibold text-gray-800 dark:text-gray-100" data-room-title>{{ __('Room __NUMBER__') }}</span>
                    </div>
                    <button type="button" class="btn-ghost-sm" data-remove-room>{{ __('Remove') }}</button>
                </div>
                <div class="mt-4 grid grid-cols-1 gap-3 md:grid-cols-12">
