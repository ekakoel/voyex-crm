...
@section('page_title', ui_phrase('modules_hotels_add_hotel'))
@section('page_subtitle', ui_phrase('modules_hotels_create_page_subtitle'))
        ...
        <div class="module-grid-8-4">
            <div class="module-grid-main">
                <form method="POST" action="{{ route('hotels.store') }}" enctype="multipart/form-data">
                    @csrf
                    @include('modules.hotels.partials._info', ['buttonLabel' => ui_phrase('modules_hotels_save_hotel'), 'destinations' => $destinations])
                </form>
            </div>
            <aside class="module-grid-side">
                <div class="module-card p-5 text-sm text-slate-600 dark:text-slate-300">
                    ...
                    <p class="mt-2">{{ ui_phrase('modules_hotels_info_text') }}</p>
                </div>
            </aside>
        </div>
    </div>
