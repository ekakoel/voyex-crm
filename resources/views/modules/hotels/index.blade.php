...
@section('page_subtitle', ui_phrase('modules_hotels_page_subtitle'))
@section('page_actions')
    <a href="{{ route('hotels.create') }}" class="btn-primary">{{ ui_phrase('modules_hotels_add_hotel') }}</a>
@endsection
@section('content')
            ...
            <aside class="module-grid-side space-y-4">
                <div class="app-card p-5 space-y-4">
                    ...
                    <form method="GET" class="grid grid-cols-1 gap-3 sm:grid-cols-2" data-hotels-index-form data-disable-submit-lock="1" data-page-spinner="off">
                        <input name="q" value="{{ request('q') }}" placeholder="{{ ui_phrase('modules_hotels_search') }}"
                        ...
                        <select name="status" class="app-input" data-hotels-filter-input>
                            <option value="">{{ ui_phrase('common_all_status') }}</option>
                            <option value="active" @selected(request('status') === 'active')>{{ ui_phrase('common_active') }}</option>
                            <option value="inactive" @selected(request('status') === 'inactive')>{{ ui_phrase('common_inactive') }}</option>
                        </select>
                        ...
                        <div class="flex items-center gap-2 sm:col-span-2 filter-actions">
                            <a href="{{ route('hotels.index') }}" class="btn-ghost" data-hotels-filter-reset>{{ ui_phrase('common_reset') }}</a>
                        </div>
                    </form>
                </div>
            </aside>
