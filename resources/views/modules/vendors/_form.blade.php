...
<div class="space-y-4">
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div>
            ...
            <input id="google_maps_url" name="google_maps_url" type="url" value="{{ old('google_maps_url', $vendor->google_maps_url ?? '') }}" placeholder="https://maps.google.com/..." class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100">
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Paste link map untuk auto-set koordinat, city, province, dan location.</p>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
            ...
            <input id="latitude" name="latitude" type="number" step="0.0000001" value="{{ old('latitude', $vendor->latitude ?? '') }}" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100" required>
        ...
        <div>
            ...
            <input id="longitude" name="longitude" type="number" step="0.0000001" value="{{ old('longitude', $vendor->longitude ?? '') }}" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100" required>
            @error('longitude') <p class="mt-1 text-xs text-rose-600">{{ $message }}</p> @enderror
        </div>
    </div>
    <div class="space-y-2">
        <p class="text-sm font-medium text-gray-700 dark:text-gray-200">Pick Vendor Location on Map</p>
        <div id="vendor-map" class="h-72 w-full rounded-lg border border-gray-300"></div>
        <p class="text-xs text-gray-500 dark:text-gray-400">Click map atau paste Google Maps URL untuk auto-fill location.</p>
    </div>
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-200">Location</label>
            <input id="location" name="location" value="{{ old('location', $vendor->location ?? '') }}" placeholder="contoh: Uluwatu, Badung" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100">
        ...
        <div>
            ...
            <input id="city" name="city" value="{{ old('city', $vendor->city ?? '') }}" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100">
        ...
        <div>
            ...
            <input id="province" name="province" value="{{ old('province', $vendor->province ?? '') }}" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100">
    ...
    @push('scripts')
        ...
        <script>
            (function () {
                ...
                const setPoint = async (lat, lng, withReverse = true) => {
                    ...
                    if (!marker) {
                        ...
                        marker.on('dragend', async (event) => {
                            ...
                            await setPoint(position.lat, position.lng, true);
                        });
                    } else {
                        marker.setLatLng([lat, lng]);
                    }

                    if (!withReverse) return;
                ...
                map.on('click', async (event) => {
                    await setPoint(event.latlng.lat, event.latlng.lng, true);
                });
