...
<div class="app-card p-5 space-y-2">
    ...
    @if ($interactive)
        <div class="flex items-center gap-2 text-xs" data-island-transfer-map-mode-group>
            <span class="text-gray-600 dark:text-gray-300">{{ ui_phrase('Pin mode:') }}</span>
            <button type="button" class="btn-outline-sm !h-8 !px-2.5" data-island-transfer-pin-mode="departure">{{ ui_phrase('Departure') }}</button>
            <button type="button" class="btn-secondary-sm !h-8 !px-2.5" data-island-transfer-pin-mode="arrival">{{ ui_phrase('Arrival') }}</button>
        </div>
    @endif
    <div
