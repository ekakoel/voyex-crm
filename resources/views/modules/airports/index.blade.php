...
@section('page_subtitle', __('ui.modules.airports.page_subtitle'))
@section('page_actions')
    <a href="{{ route('airports.create') }}" class="btn-primary">{{ __('ui.modules.airports.add_airport') }}</a>
@endsection
@section('content')
        ...
        <div class="module-grid-3-9">
            <aside class="module-grid-side space-y-4">
                <div class="app-card p-5">
                    <div>
                        <h2 class="text-base font-semibold text-gray-800 dark:text-gray-100">{{ __('ui.common.filters') }}
                        </h2>
                        ...
                        <table class="app-table w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                            <thead>
                                <tr>
                                    ...
                                    <th
                                        class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300">
                                        {{ __('ui.modules.airports.airport') }}</th>
                                    <th
                                        class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300">
                                        ...
                                        <td class="px-4 py-3 text-right text-sm actions-compact">
                                            <div class="flex items-center justify-end gap-2">
                                                <a href="{{ route('airports.show', $airport) }}" class="btn-outline-sm"
                                                    title="{{ __('ui.common.view') }}"
                                                    aria-label="{{ __('ui.common.view') }}"><i
                                                ...
                                                <a href="{{ route('airports.edit', $airport) }}" class="btn-secondary-sm"
                                                    ...
                                                    aria-label="{{ __('ui.common.edit') }}"><i
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6"
                                            ...
                                            {{ __('ui.index.no_data_available', ['entity' => __('ui.entities.airports')]) }}
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                    ...
                    @forelse ($airports as $airport)
                        <div class="app-card p-4">
                            ...
                            <div class="mt-3 flex flex-wrap gap-2">
                                <a href="{{ route('airports.show', $airport) }}" class="btn-outline-sm"
                                    title="{{ __('ui.common.view') }}" aria-label="{{ __('ui.common.view') }}"><i
                                        class="fa-solid fa-eye"></i><span
                                        class="sr-only">{{ __('ui.common.view') }}</span></a>
                                <a href="{{ route('airports.edit', $airport) }}" class="btn-secondary-sm"
                                    title="{{ __('ui.common.edit') }}" aria-label="{{ __('ui.common.edit') }}"><i
                                        class="fa-solid fa-pen"></i><span
                                        class="sr-only">{{ __('ui.common.edit') }}</span></a>
                                <form action="{{ route('airports.toggle-status', $airport->id) }}" method="POST"
                                    ...
                                    @method('PATCH')
                                    <button type="submit"
                                        onclick="return confirm('{{ $airport->trashed() ? __('ui.modules.airports.confirm_activate') : __('ui.modules.airports.confirm_deactivate') }}')"
                                        class="{{ $airport->trashed() ? 'btn-primary-sm' : 'btn-muted-sm' }}">{{ $airport->trashed() ? __('ui.common.activate') : __('ui.common.deactivate') }}</button>
                                </form>
                            </div>
                        </div>
                    @empty
                        <div class="app-card p-6 text-center text-sm text-gray-500 dark:text-gray-400">
                            {{ __('ui.index.no_data_available', ['entity' => __('ui.entities.airports')]) }}</div>
                    @endforelse
                </div>
