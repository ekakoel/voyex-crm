...
@section('page_title', ui_phrase('modules_transports_create_page_title'))
@section('page_subtitle', ui_phrase('modules_transports_create_page_subtitle'))
        ...
        <div class="module-grid-8-4">
            <div class="module-grid-main">
                <div class="module-form-wrap">
                    <form method="POST" action="{{ route('transports.store') }}" enctype="multipart/form-data">
                        @csrf
                        @include('modules.transports._form', ['buttonLabel' => ui_phrase('modules_transports_save_transport')])
                    </form>
                </div>
            </div>
            <aside class="module-grid-side">
                <div class="module-card p-5 text-sm text-slate-600 dark:text-slate-300">
                    ...
                    <p class="mt-2">{{ ui_phrase('modules_transports_info_text') }}</p>
                </div>
            </aside>
        </div>
    </div>
