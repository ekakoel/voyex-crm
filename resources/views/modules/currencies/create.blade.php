...
@section('page_actions')
    <a href="{{ route('currencies.index') }}" class="btn-ghost" data-page-back-action>{{ ui_phrase('Back') }}</a>
@endsection
            ...
            <div class="module-grid-main">
                <div class="module-form-wrap">
                    <form method="POST" action="{{ route('currencies.store') }}">
                        @csrf
                        @include('modules.currencies._form', ['buttonLabel' => ui_phrase('Create Currency')])
                    </form>
                </div>
            </div>
