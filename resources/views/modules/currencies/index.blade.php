...
@section('content')
    <div class="space-y-6 module-page module-page--currencies" data-service-filter-page data-page-spinner="off">
        ...
        <div class="grid grid-cols-1 gap-6 xl:grid-cols-12">
            ...
            <div class="flex items-center gap-2 sm:col-span-2 filter-actions">
                <a href="{{ route('currencies.index') }}" class="btn-ghost" data-service-filter-reset>{{ ui_phrase('Reset') }}</a>
            </div>
        </form>
                </div>
            </aside>
            ...
            <table class="app-table w-full divide-y divide-gray-200 text-sm dark:divide-gray-700">
                <thead>
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300">{{ ui_phrase('Symbol') }}</th>
                        <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300">{{ ui_phrase('Rate to IDR') }}</th>
                        <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300">{{ ui_phrase('Decimals') }}</th>
    ...
    <div class="flex items-center justify-end gap-2">
        <a href="{{ route('currencies.edit', $currency) }}"  class="btn-secondary-sm" title="{{ ui_phrase('Edit') }}" aria-label="{{ ui_phrase('Edit') }}"><i class="fa-solid fa-pen"></i><span class="sr-only">{{ ui_phrase('Edit') }}</span></a>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-400">{{ ui_phrase('No :entity available.', ['entity' => ui_phrase('Currencies')]) }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            @empty
                <div class="app-card p-6 text-center text-sm text-gray-500 dark:text-gray-400">{{ ui_phrase('No :entity available.', ['entity' => ui_phrase('Currencies')]) }}</div>
            @endforelse
        </div>
