...
@section('content')
    <div class="space-y-6 module-page module-page--currencies">
        @section('page_actions')<a href="{{ route('currencies.index') }}"  class="btn-ghost" data-page-back-action>{{ ui_phrase('Back') }}</a>@endsection
        ...
        <div class="module-grid-8-4">
            <div class="module-grid-main">
                <div class="module-form-wrap">
                    <form method="POST" action="{{ route('currencies.update', $currency) }}">
                        @csrf
                        @method('PUT')
                        @include('modules.currencies._form', ['currency' => $currency, 'buttonLabel' => ui_phrase('Update Currency')])
                    </form>
                </div>
            </div>
            <aside class="module-grid-side space-y-4">
                <div class="module-card p-6">
                            ...
                            @foreach ($rateHistories as $history)
                                <div class="rounded-lg mb-6 border border-gray-200 bg-gray-50 px-3 py-2 dark:border-gray-700 dark:bg-gray-900/40">
                                    <div class="flex items-center justify-between gap-3">
                                        <span class="font-semibold">{{ ui_phrase('IDR Rate') }}</span>
                    ...
                    @else
                        <p class="mt-3 text-xs text-gray-500 dark:text-gray-400">{{ ui_phrase('no rate changes') }}</p>
                    @endif
                </div>
            </aside>
        </div>
