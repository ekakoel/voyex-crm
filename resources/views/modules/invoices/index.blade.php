...
@section('page_title', ui_phrase('modules_invoices_page_title'))
@section('page_subtitle', ui_phrase('modules_invoices_page_subtitle'))
@section('content')
    <div class="space-y-6 module-page module-page--invoices" data-service-filter-page data-page-spinner="off">
        ...
        <div class="grid grid-cols-1 gap-6 xl:grid-cols-12">
            ...
            <input name="q" value="{{ request('q') }}" placeholder="{{ ui_phrase('modules_invoices_search') }}" class="app-input sm:col-span-2" data-service-filter-input>
            ...
            <div class="flex items-center gap-2 sm:col-span-2 filter-actions">
                <a href="{{ route('invoices.index') }}" class="btn-ghost" data-service-filter-reset>{{ ui_phrase('common_reset') }}</a>
            </div>
        </form>
                </div>
            </aside>
            ...
            <table class="app-table w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                <thead>
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300">{{ ui_phrase('common_invoice') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300">{{ ui_phrase('common_booking') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300">{{ ui_phrase('common_customer') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300">{{ ui_phrase('modules_invoices_invoice_date') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300">{{ ui_phrase('modules_invoices_due_date') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300">{{ ui_phrase('common_amount') }}</th>
    ...
    <div class="flex items-center justify-end gap-2">
        <a href="{{ route('invoices.show', $invoice) }}" class="btn-secondary-sm" title="{{ ui_phrase('common_detail') }}" aria-label="{{ ui_phrase('common_detail') }}"><i class="fa-solid fa-eye"></i><span class="sr-only">{{ ui_phrase('common_detail') }}</span></a>
    </div>
</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="9" class="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-400">{{ ui_phrase('index_no_data_available', ['entity' => ui_phrase('entities_invoices')]) }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="md:hidden space-y-3">
            @forelse ($invoices as $invoice)
                <div class="app-card p-4">
                    ...
                    <div class="mt-3 grid grid-cols-2 gap-2 text-xs text-gray-600 dark:text-gray-300">
                        <div>{{ ui_phrase('modules_invoices_invoice_date') }}</div>
                        <div>{{ $invoice->invoice_date?->format('Y-m-d') ?? '-' }}</div>
                        <div>{{ ui_phrase('modules_invoices_due_date') }}</div>
                        <div>{{ $invoice->due_date?->format('Y-m-d') ?? '-' }}</div>
                        <div>{{ ui_phrase('common_amount') }}</div>
            ...
            @empty
                <div class="app-card p-6 text-center text-sm text-gray-500 dark:text-gray-400">{{ ui_phrase('index_no_data_available', ['entity' => ui_phrase('entities_invoices')]) }}</div>
            @endforelse
        </div>
